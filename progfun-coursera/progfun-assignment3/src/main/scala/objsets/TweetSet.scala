...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = 
  {
      ...
      if( elem.retweets > mostRetweetedLeft.retweets && elem.retweets > mostRetweetedRight.retweets ) elem
      else if( mostRetweetedLeft.retweets > mostRetweetedRight.retweets ) mostRetweetedLeft
      else mostRetweetedRight
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = allTweets.filter( tweet => google.exists( text => tweet.text contains text ) )
  lazy val appleTweets: TweetSet = allTweets.filter( tweet => apple.exists( text => tweet.text contains text ) )
