...
object Huffman {
  ...
  {
    case List() => List()
  	case x :: xs => countPair( (x, 1), times( xs ) ) 
  }
  
  def countPair(pair: (Char, Int), acc: List[(Char, Int)]): List[(Char, Int)] = acc match
  {
    case List() => List(pair)
    case x :: xs => if( x._1 == pair._1 ) List( (x._1, x._2+pair._2) ) ::: xs 
    				else List( x ) ::: countPair(pair, xs)
  }
  ...
  def insertLeaf( theLeaf: Leaf, acc: List[Leaf] ): List[Leaf] = acc match
  {
    case List() => List( theLeaf )
    case x :: xs => if( theLeaf.weight <= x.weight ) List(theLeaf) ::: acc
    				else List( x ) ::: insertLeaf( theLeaf, xs )
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match
  {
    case List() => trees
    case x :: Nil => trees
    case x :: y :: xs => insertNode( Fork( x, y, chars(x) ::: chars(y), weight(x) + weight(y) ), xs )
  }
  
  def insertNode( theNode: Fork, acc: List[CodeTree] ): List[CodeTree] = acc match
  {
    case List() => List( theNode )
    case x :: xs => x match
    				{
    					case Leaf(lc, lw) => if( theNode.weight <= lw ) List(theNode) ::: acc
    										 else List( x ) ::: insertNode( theNode, xs )
    					case Fork( fl, fr, fc, fw ) => if( theNode.weight <= fw ) List(theNode) ::: acc
    										 		   else List( x ) ::: insertNode( theNode, xs )
    				}
  }
   ...
   */
  def until( isSingleton: List[CodeTree] => Boolean, combinedSoFar: List[CodeTree]=>List[CodeTree] )(trees: List[CodeTree]): List[CodeTree] = 
		  if( isSingleton(trees) ) trees
		  else until( isSingleton, combinedSoFar )( combinedSoFar(trees) )
  ...
  {
  	  case Leaf( c, w ) => List()
  	  case Fork( l, r, fc, fw ) => if( containsChar(l, char) ) List(0) ::: encodeChar( l )( char ) else List(1) ::: encodeChar( r )( char )
  }
  
  def containsChar( tree: CodeTree, char: Char ): Boolean = tree match
  {
      case Leaf( c, w ) => c == char
      case Fork( l, r, fc, fw ) => fc.contains( char )
  }
